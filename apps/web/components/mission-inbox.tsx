...
export function MissionInbox() {
  ...
  if (!items && !error) return <section className="card inbox-loading" role="status" aria-live="polite" aria-busy="true">
    <div><span className="mission-bucket">{tr("Connecting", "Conectando", "Conectando")}</span><small>{tr("Checking runtime state", "Revisando el estado de ejecución", "Verificando o estado de execução")}</small></div>
  ...
  return <div className="mission-inbox">
    ...
    {items?.map((item) => <a className={`mission-inbox-card ${item.attentionRequired ? "needs-you" : ""}`} href={localize(item.detailPath ?? `/missions/${item.missionId}/result`)} key={item.missionId}>
      <div><span className="mission-bucket">{item.bucket === "NEEDS_YOU" ? tr("Intervention", "Intervención", "Intervenção") : item.bucket === "DONE" ? tr("Verified", "Verificada", "Verificada") : tr("Running", "En curso", "Em andamento")}</span><small>{item.channelLabel}</small></div>
      <h2>{item.companyName}</h2><p className="mission-outcome">{item.outcomeLabel}</p>
