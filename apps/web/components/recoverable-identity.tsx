...
export function RecoverableIdentity({
  ...
  if (state === "RECOVERABLE") return <div className="identity-gate ready"><span aria-hidden="true">✓</span><div><strong>{tr("Access saved", "Acceso guardado", "Acesso salvo")}</strong><p>{email ? `${tr("Continue with", "Continuar con", "Continuar com")} ${email}.` : tr("This case can be reopened after sign-in on another device.", "Este caso puede reabrirse después de iniciar sesión en otro dispositivo.", "Este caso pode ser reaberto após entrar em outro dispositivo.")}</p></div></div>;
  return <div className="identity-gate" data-required={required}>
    <span aria-hidden="true">↗</span><div><strong>{required ? tr("Save access before DueBack sends", "Guardá el acceso antes de que DueBack envíe", "Salve o acesso antes do envio") : tr("Save access to your follow-ups", "Guardá el acceso a tus seguimientos", "Salve o acesso aos seus acompanhamentos")}</strong>
      ...
      <button type="button" className="secondary" disabled={busy} onClick={() => {
        ...
        void linkCurrentIdentityWithGoogle().then((result) => {
          ...
          setError(code === "RECOVERABLE_ACCOUNT_ALREADY_EXISTS"
            ? tr("That Google account already has DueBack access. This draft remains safely in this browser; automatic case merging is not enabled yet.", "Esa cuenta Google ya tiene acceso a DueBack. El borrador permanece seguro en este navegador; todavía no se combinan casos automáticamente.", "Essa conta Google já possui acesso ao DueBack. O rascunho permanece seguro neste navegador; os casos ainda não são combinados automaticamente.")
            : code === "RECOVERABLE_SIGN_IN_CANCELLED" ? tr("Sign-in was cancelled. Nothing was sent.", "Se canceló el inicio de sesión. No se envió nada.", "O login foi cancelado. Nada foi enviado.") : tr("DueBack could not save access. Nothing was sent.", "DueBack no pudo guardar el acceso. No se envió nada.", "O DueBack não conseguiu salvar o acesso. Nada foi enviado."));
        }).finally(() => {
          setBusy(false);
        });
