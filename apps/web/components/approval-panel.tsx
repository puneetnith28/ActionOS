...
export function ApprovalPanel(props: {
  ...
  const blockerCopy: Record<Exclude<BlockerReason, null>, string> = {
    ...
    contact: tr("Confirm the authorized contact to enable boundary.", "Confirmá el contacto autorizado para habilitar la aprobación.", "Confirme o contato autorizado para habilitar a aprovação."),
  ...
  return <div className="boundary.primary-panel">
    <div className="boundary.primary-heading"><span className="status-dot" aria-hidden="true" /><div><small>{tr("PLAN", "PLAN", "PLANO")} v{props.planVersion}</small><strong>{tr("Approve this follow-up", "Aprobá este seguimiento", "Aprove este acompanhamento")}</strong></div></div>
    <dl className="boundary.primary-facts">
    ...
    <div className="boundary.proof"><small>{tr("DONE ONLY WHEN", "TERMINA SÓLO CUANDO", "CONCLUI SOMENTE QUANDO")}</small><strong>{props.proofRequired}</strong></div>
    {props.controlled ? <p className="controlled-context"><strong>{tr("Controlled demo", "Demo controlada", "Demonstração controlada")}</strong><span>{tr("ActionOS contacts its isolated simulator. No real company is contacted.", "ActionOS contacta su simulador aislado. No se contacta ninguna empresa real.", "O ActionOS contata seu simulador isolado. Nenhuma empresa real é contatada.")}</span></p> : null}
    ...
    <button className="primary boundary.primary-cta" type="button" disabled={props.busy || props.disabled} onClick={props.onApprove}>{props.actionLabel}</button>
    {props.blockerReason ? <p className="button-help">{blockerCopy[props.blockerReason]}</p> : <p className="button-help boundary.ready">{tr("Nothing has been sent yet.", "Todavía no se envió nada.", "Nada foi enviado ainda.")}</p>}
  </div>;
}
