...
export function CaseResult({ caseId }: { readonly caseId: string }) {
  ...
  return <div className="result-grid consumer-case-detail">
    {error ? <section className="card error refresh-warning" role="alert"><p>{tr("DueBack could not refresh. Your last saved state remains below.", "DueBack no pudo actualizar. El último estado guardado continúa abajo.", "O DueBack não conseguiu atualizar. O último estado salvo continua abaixo.")}</p><button type="button" onClick={() => { setRefreshKey((value) => value + 1); }}>{tr("Try again", "Intentar nuevamente", "Tentar novamente")}</button></section> : null}
    ...
    {detail.comparison.length ? <section className="card outcome-comparison" aria-labelledby="comparison-title"><div className="eyebrow">{tr("Proof check", "Verificación de prueba", "Verificação da prova")}</div><h2 id="comparison-title">{tr("Promised vs. observed", "Prometido vs. observado", "Prometido vs. observado")}</h2><p>{tr("Missing facts stay missing; DueBack never copies them from the promise.", "Los datos ausentes siguen ausentes; DueBack nunca los copia de la promesa.", "Dados ausentes continuam ausentes; o DueBack nunca os copia da promessa.")}</p><div className="comparison-table" role="table">{detail.comparison.map((row) => <div role="row" key={row.label} data-status={row.status}><strong role="cell">{row.label}</strong><span role="cell">{row.promised}</span><span role="cell">{row.observed}<small>{row.status === "MATCH" ? tr(" Matches", " Coincide", " Corresponde") : row.status === "MISSING" ? tr(" Missing", " Ausente", " Ausente") : tr(" Different", " Diferente", " Diferente")}</small></span></div>)}</div></section> : null}
    <section className="card case-conversation" aria-labelledby="conversation-title"><div className="eyebrow">{tr("Conversation", "Conversación", "Conversa")}</div><h2 id="conversation-title">{tr("What DueBack and the company said", "Qué dijeron DueBack y la empresa", "O que o DueBack e a empresa disseram")}</h2>{detail.conversation.length ? <ol>{detail.conversation.map((entry) => <li key={entry.id} data-direction={entry.direction}><div><strong>{entry.title}</strong>{entry.occurredAt ? <time dateTime={entry.occurredAt}>{new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short" }).format(new Date(entry.occurredAt))}</time> : null}</div><p className="conversation-body">{entry.safeBody}</p><p className="conversation-reason"><span>{entry.status.replaceAll("_", " ")}</span>{entry.reason}</p></li>)}</ol> : <p>{tr("DueBack has not contacted the company yet.", "DueBack todavía no contactó a la empresa.", "O DueBack ainda não contatou a empresa.")}</p>}</section>
    <section className="card"><h2>{tr("Case controls", "Controles del caso", "Controles do caso")}</h2><p>{tr("Stop future actions or report that an accepted result did not arrive.", "Detené acciones futuras o informá que un resultado aceptado no llegó.", "Interrompa ações futuras ou informe que um resultado aceito não chegou.")}</p><a className="button-link" href={localize(`/cases/${caseId}/exception`)}>{detail.outcome.accepted ? tr("This isn't resolved", "Esto no está resuelto", "Isto não está resolvido") : tr("Review or stop this case", "Revisar o detener este caso", "Revisar ou interromper este caso")}</a></section>
