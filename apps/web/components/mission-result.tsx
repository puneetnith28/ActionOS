...
import { MissionExport } from "./mission-export";
...
export function MissionResult({ missionId }: { readonly missionId: string }) {
  ...
  if (error && !detail) return <section className="card error mission-access-error" role="alert"><h2>{tr("Sign in to open this private case", "Iniciá sesión para abrir este caso privado", "Entre para abrir este caso privado")}</h2><p>{tr("The link does not grant access. Use the Google account that owns this follow-up.", "El enlace no concede acceso. Usá la cuenta Google propietaria del seguimiento.", "O link não concede acesso. Use a conta Google proprietária do acompanhamento.")}</p><GoogleSignIn onSignedIn={() => { setRefreshKey((value) => value + 1); }} /><button type="button" onClick={() => { setRefreshKey((value) => value + 1); }}>{tr("Try current session", "Probar la sesión actual", "Tentar a sessão atual")}</button></section>;
  if (!detail) return <section className="card mission-loading" role="status" aria-live="polite" aria-busy="true">
    ...
    <div className="mission-loading-steps" aria-hidden="true"><span /><span /><span /></div>
  </section>;
  ...
  return <div className="result-grid consumer-mission-detail">
    ...
    <section className="card mission-conversation" aria-labelledby="conversation-title"><div className="eyebrow">{tr("Agent Activity", "Actividad del Agente", "Atividade do Agente")}</div><h2 id="conversation-title">{tr("Capability Execution Logs", "Registros de ejecución", "Logs de execução")}</h2>{detail.conversation.length ? <ol>{detail.conversation.map((entry) => <li key={entry.id} data-direction={entry.direction}><div><strong>{entry.title}</strong>{entry.occurredAt ? <time dateTime={entry.occurredAt}>{new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short" }).format(new Date(entry.occurredAt))}</time> : null}</div><p className="conversation-body">{entry.safeBody}</p><p className="conversation-reason"><span>{entry.status.replaceAll("_", " ")}</span>{entry.reason}</p></li>)}</ol> : <p>{tr("The agent has not invoked any capabilities yet.", "El agente no ha invocado capacidades aún.", "O agente ainda não invocou capacidades.")}</p>}</section>
