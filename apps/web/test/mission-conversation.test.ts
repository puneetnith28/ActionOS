...
describe("consumer conversation projection", () => {
  it("explains acknowledgement without inventing missing outcome facts", () => {
    ...
    const entries = missionConversation(item, evidence, [{ acceptedAt: "2026-08-17T10:00:00.000Z", transportStatus: "DELIVERED" }]);
  ...
  it("formats minor currency units as consumer money", () => {
    const evidence = [{
      candidate: {
        outcomeId: "evidence_money_1234", missionId: "mission_money_12345678",
        status: "OUTCOME_CONFIRMED", transactionRef: "R-59", amountMinor: 5900,
    ...
    const entries = missionConversation(item, evidence, []);
