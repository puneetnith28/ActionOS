const messages: Readonly<Record<string, string>> = {
  ...
  DAILY_CASE_BUDGET_EXHAUSTED: "This demo has reached today's mission limit for this session.",
  MODEL_CALL_BUDGET_EXHAUSTED: "This mission reached its model-call limit. Start a new mission or try tomorrow.",
  ...
  MALFORMED_FILE: "ActionOS could not read that file. Export it again or upload a screenshot.",
  PROMISE_SOURCE_REQUIRED: "Paste a promise or choose a file to continue.",
  PROMISE_PLAN_INVALID: "ActionOS read the promise but could not build a safe plan. Add the company, reference, expected result, and due date, then try again.",
  ...
  VERIFIED_NOTIFICATION_EMAIL_REQUIRED: "Mission updates must use the verified email on your Google identity.",
  PLAN_REQUEST_FAILED: "We could not load this plan. Please retry.",
  REQUEST_FAILED: "ActionOS could not complete that request. Please try again."
};

export function errorCopy(code: string): string {
  return messages[code] ?? "ActionOS could not complete that request. Please try again.";
}
