...
const detail = {
  missionId: "mission_consumer_12345678", version: 3, state: "WAITING_EXTERNAL", statusLabel: "Waiting for proof",
  ...
  channel: { type: "MANAGED_EMAIL", label: "Email", disclosure: "Controlled email pilot", contact: "ActionOS sends the approved email", reply: "The company replies by email", recipientHint: "m•••@example.test" },
  returnPath: "Durable mission page", outcome: { accepted: false, acknowledgementOnly: true, title: "Not done — request received only", explanation: "ActionOS keeps this open until explicit evidence meets the approved contract.", limitation: "Bank settlement is not verified." },
...
test.describe("consumer case detail", () => {
  ...
  test("renders a legacy-safe projection without technical or private fields", async ({ page }) => {
    await page.route("**/api/missions/mission_consumer_12345678/detail", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(detail) }));
    await page.goto(`${deployedUrl}/missions/mission_consumer_12345678/result`);
  ...
  test("preserves the last known case when refresh fails", async ({ page }) => {
    let calls = 0;
    await page.route("**/api/missions/mission_consumer_12345678/detail", async (route) => {
      calls += 1;
