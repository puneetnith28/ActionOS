...
test.describe("durable attention return", () => {
  ...
  test("a closed-tab mission returns in My follow-ups as Needs you", async ({ context }) => {
    await context.route("**/api/missions?*", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ items: [{ missionId: "mission_attention_12345678", companyName: "Northstar", outcomeLabel: "USD 59 refund", bucket: "NEEDS_YOU", statusLabel: "Decision needed", lastActivityAt: "2026-08-17T10:02:00.000Z", nextStepLabel: "Review one decision", attentionRequired: true, channelLabel: "Email" }], nextCursor: null }) }));
    ...
    await returnPage.goto(`${deployedUrl}/missions`);
