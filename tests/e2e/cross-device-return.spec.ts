...
const missionId = process.env.ACTIONOS_CROSS_DEVICE_MISSION_ID;

test.describe("real cross-device ownership", () => {
  test.skip(!deployedUrl || !ownerState || !missionId, "Provide a real Firebase owner state and owned mission; this claim gate cannot use mocked identity");
  test("the linked Google owner recovers the mission and another owner sees no facts", async ({ browser }) => {
    const owner = await browser.newContext({ storageState: ownerState });
    const ownerPage = await owner.newPage();
    await ownerPage.goto(`${deployedUrl}/missions/${missionId}/result`);
    ...
    await otherPage.goto(`${deployedUrl}/missions/${missionId}/result`);
